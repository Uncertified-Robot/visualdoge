...
Floatable.prototype.addImage=function(a,d,g){this.canvas=document.createElement("canvas");this.image=a;this.canvas.height=g;this.canvas.width=d;this.canvas.style.position="absolute";this.canvas.style.top="0px";this.canvas.style.left="0px";this.canvas.getContext("2d").drawImage(this.image,0,0,d-1,g-1);this.div.appendChild(this.canvas);console.log(this)};Floatable.prototype.addText=function(a){this.innerDiv.innerHTML+=a};
...
transform:"scaleX(-1)"});-.1>a.velocity.x&&$(a.div).css({"-moz-transform":"scaleX(1)","-o-transform":"scaleX(1)","-webkit-transform":"scaleX(1)",transform:"scaleX(1)"})}};new Konami(easterSuccess);function Block(a,d,g,l){"visible"===document.visibilityState&&(Floatable.call(this),l=Math.floor(l/1024)+" KB",this.width=this.height=500,this.addImage(blockImage,this.width,this.height),this.addText("Block #"+a+"<br />Number of Transactions: "+d+"<br />Difficulty: "+g+"<br />Block Size: "+l),this.initPosition());Sound.playRandomSwell()}extend(Floatable,Block);function Transaction(a,d,g,l){if("visible"===document.visibilityState){Floatable.call(this);this.area=a/1;this.width=this.height=1*Math.sqrt(this.area/Math.PI);1E3<this.width&&(this.width=this.height=1E3);75>this.width&&(this.width=this.height=75);this.addImage(bubbleImage,this.width,this.height);var h="&ETH;"+a.toFixed(2);"&ETH;0.00"==h&&(h="<&ETH;0.01");d?this.addText('<span style="color: yellow;">'+h+'</span><br /><span style="color: cyan;">Donation</span><br /><span style="color: lime;">Thanks!</span>'):
this.addText(h);g&&l&&this.addText("<br />"+g.toFixed(2)+" "+l);this.initPosition()}d=a/(1E6/(.7-.3))+.3;.7<d&&(d=.7);a=Math.min(100,Math.log(a+1.0715307808111487)/Math.log(1.0715307808111487));globalScalePitch?Sound.playPitchAtVolume(d,100-a):Sound.playRandomAtVolume(d)}extend(Floatable,Transaction);var DEBUG_MODE=!1,DONATION_ADDRESS,SOUND_DONATION_ADDRESS,globalMute=!1,instanceId=0,pageDivId="pageDiv",last_update=0,updateTargets=[],bubbleImage=new Image;bubbleImage.src="images/bubble.png";var blockImage=new Image;blockImage.src="images/block.png";var debugSpawner,updateLayoutWidth=function(){$(".chartMask").css("visibility","visible")},updateLayoutHeight=function(){var a=window.innerHeight;"none"!=$("#header").css("display")&&(a-=$("#header").outerHeight());$("#pageSplitter").height(a)};
